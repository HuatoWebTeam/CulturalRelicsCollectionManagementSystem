...
import { Row, Col, Button, Form, DatePicker, Input, Table } from "antd";
import "./index.less";
import { Link } from 'react-router-dom';
import { RangePickerDefault, approveState, subStr, howComplete } from "../../assets/js/commonFun";
import moment from 'moment';
import { CancellationAll } from './api';
    ...
    CancellationAll(params).then(res => {
      ...
      if(res.length > 0) {
        for (let item of res) {
          ...
          item.disabled = Number(item.StepState) === 4 ? (Number(item.Step) === 0 ? true : false) : Number(item.StepState) === 1 ? (Number(item.Step) === 1 ? true : false) : false;
        }
        this.setState({
          logoutList: res,
          total: res[0].cou
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'Collection_img1',
        render: (text) => {
          return <img style={{width: '55px', height: '55px'}} src={text} alt='img' />
        }
      },
      {
        title: '文物等级',
      ...
      },
      {
        ...
        render: (text) => {
          return <span>
              ...
              <Button disabled={!text.disabled} type="button" style={{ border: "none", marginLeft: "10px" }}>
                删除
              </Button>
            </span>;
    ...
    return <Row className="main-content">
        ...
        <Col className="logout-list-content" span={24} style={{padding: '20px 40px 20px 20px'}} >
            ...
            <Col span={20}>
              <Form layout="inline" style={{ float: "right" }}>
                <Form.Item label="选择注销日期:">
                  <RangePicker onChange={this.datePickerChange} defaultValue={RangePickerDefault} />
                </Form.Item>
                <Form.Item>
                  <Search placeholder="请输入文物名称" enterButton onSearch={this.searchData} />
                </Form.Item>
              </Form>
            </Col>
          </Col>
